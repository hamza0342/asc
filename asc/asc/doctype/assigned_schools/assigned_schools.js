...
var total_school = 0;
frappe.ui.form.on('Assigned Schools', {
	onload: function (frm) {

		frm.disable_save();
		frappe.call({
			method: "asc.asc.doctype.assigned_schools.assigned_schools.get_schools",
			async: false,
			callback: function (r) {
	...
	for (var i = 0; i < data[0].length; i++) {
		...
		if (data[0][i]['asc_name'].length > 0) {
			if (data[0][i]['completion_check'] == 1) {
				...
				rows_string += "<a class='btn btn-success' style='background-color: #00a1ff;' href= '/app/asc/" + String(data[0][i]['asc_name']) + "'>Completed</a>";

			}
			if (data[0][i]['completion_check'] == 0) {
				rows_string += "<a class='btn btn-success' style='background-color: #ff7900;'; href= '/app/asc/" + String(data[0][i]['asc_name']) + "'>Edit/Finalize</a>";
			}
		}
		else {
			rows_string += `<a class="btn btn-primary" style="background-color:#FF0000;"; onclick="frappe.set_route('asc/new-asc-1',{'semis_code': ` + String(data[0][i]['school']) + `})">Enter Data</a>`
		}
		rows_string += `</td></tr>`;
	}
	return rows_string;
...
function get_table(data, rows) {
	var panding = data[0].length - data[1][0]['census_completed'] - data[1][0]['incomplete_form'] - data[1][0]['verified']
...
<span class="value_count">`+ data[1][0]['verified'].toLocaleString("en-US") + `</span><br>
