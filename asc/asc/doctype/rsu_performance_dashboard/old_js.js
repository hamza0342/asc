	...
	if (data[4].length > 0) {
		for (var i = 0; i < data[4].length; i++) {
			...
			for (var j = 0; j < data[4].length; j++) {
				if (data[4][j]["district"] == data[4][i]['district']) {
					assigned = data[4][j]["assigned_district"];
					break;
				}
			}

			var verified_t = 0, completed_t = 0, not_completed_t = 0;
			for (var j = 0; j < data[5].length; j++) {
				if (data[5][j]["district"] == data[4][i]['district']) {
					verified_t = data[5][j]["verified"];
					completed_t = data[5][j]["completed"];
					not_completed_t = data[5][j]["not_completed"];
					break;
				}
			}
			var pending = 0, not_completed = 0, completed = 0, verified = 0;
			if (data[4].length > 0) {
				var pending = assigned - (completed_t + verified_t + not_completed_t);
				var not_completed = not_completed_t;
				var completed = completed_t;
				var verified = verified_t;
			}
			rows_string += `<td style="border:1px solid #000000;text-align:center">` + assigned + `</td>
			<td style="border:1px solid #000000;text-align:center">` + completed + `</td>
			<td style="border:1px solid #000000;text-align:center">` + not_completed + `</td>
			<td style="border:1px solid #000000;text-align:center">` + verified + `</td>
...
function get_table(rows, data) {
	...
	if (data[4].length > 0) {
		...
		var pending = data[1]['assigned_schools'] - (data[2][0]["completed"] + data[2][0]['verified'] + data[2][0]["not_completed"])
