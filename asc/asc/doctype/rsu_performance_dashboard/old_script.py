...
def get_assigned_district(default_year):
	temp_query="SELECT distinct ros.district, count(per.school) as assigned_district  FROM `tabASC Roster` ros inner join `tabSchool Permission` per on ros.name = per.parent where ros.year='%s' group by ros.district"""%(default_year)
	district = frappe.db.sql(temp_query,as_dict=1)
	return district

def get_completed_district(default_year,school_string):
	temp_query= """SELECT district, SUM(CASE WHEN docstatus=0 AND completion_check=1 THEN 1 ELSE 0 END) AS completed,SUM(CASE WHEN docstatus=0 AND completion_check=0 THEN 1 ELSE 0 END) AS not_completed,SUM(CASE WHEN docstatus=1 THEN 1 ELSE 0 END) AS verified FROM tabASC where  year='%s' %s group by district """%(default_year,school_string)
	district = frappe.db.sql(temp_query,as_dict=1)
	return district
