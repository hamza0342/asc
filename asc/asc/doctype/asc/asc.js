...
frappe.ui.form.on('ASC', {
	...
	lat_n: function (frm) {
		...
		if (parseFloat(frm.doc.lat_n) < 24.0 || parseFloat(frm.doc.lat_n) > 30.0) {
			frm.set_value("lat_n", '')
			frappe.msgprint("Latitude Must be between 24 to 30")
		}
	},
	lon_e: function (frm) {

		if (parseFloat(frm.doc.lon_e) < 66.0 || parseFloat(frm.doc.lon_e) > 72.0) {
			frm.set_value("lon_e", '')
			frappe.msgprint("Longitude Must be between 66 to 72")
		}
	},
	/* End of Section 0 */
	...
	ena_constituency: function (frm) {
		if (!Number.isInteger(frm.doc.ena_constituency)) {
			...
			frappe.msgprint("NA Constituency Must be a valid Number")
	...
	fps_constituency: function (frm) {
		if (!Number.isInteger(frm.doc.fps_constituency)) {
			...
			frappe.msgprint("PS Constituency Must be a valid Number")
	...
	// 				$(".msgprint").empty()
	// 				frappe.msgprint("Please Enter valid school phone number")
	// 				frm.set_value("school_phone_no", '')
	// 			}
	// 		});
	// 	}
	...
	onload: function (frm) {
		...
		$("input[data-fieldname='lat_n']").mask('00.000000');
		$("input[data-fieldname='lon_e']").mask('00.000000');
...
// frappe.ui.form.on("Teaching Staff", {
