...
class ASCRoster(Document):
	...
	def assignment(self):
		schools = []
		if self.school_permission:
			for row in self.school_permission:
				schools.append(row.school)
		frappe.msgprint(frappe.as_json(schools))
		default_year = frappe.db.get_single_value("ASC Panel", "default_year")
		asc_names =[]

		for school in schools:
			temp_query = """Select name from tabASC where year = '%s' and semis_code = '%s' """ % (
					default_year,school)
			form_exist = frappe.db.sql(temp_query)
			if form_exist:
				frappe.throw("Form already exists for %s"%(school))
			else:
				asc = frappe.new_doc("ASC")
				asc.year = default_year
				asc.semis_code = school
				asc.save(ignore_permissions=True)
				asc_names.append(asc.name)
		frappe.msgprint(frappe.as_json(self.user))
		for asc in asc_names:
			args = {
				"assign_to": str(self.user),
				"doctype": 'ASC',
				"name": asc,
				"write":1
			}
