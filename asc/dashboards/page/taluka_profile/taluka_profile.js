frappe.pages['taluka-profile'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Taluka Profile',
		single_column: true
	});
	filters.add(page);
...
filters = {
	...
	add: function (page) {
		...
		let district_ = page.add_field({
			label: "District",
			fieldtype: "Link",
			fieldname: "district",
			options: "District",
			onchange() {
				if (district_.get_value()) {
					district.set_value("")
				}
			}
			// reqd: 1,
		...
		district = page.add_field({
			...
			reqd: 1,
			"get_query": function () {
				var division = district_.get_value();
				if (division) {
					return {
						"doctype": "Taluka",
						"filters": {
		...
		let fiterbtn = page.add_field({
			...
			async click() {
				if (district.get_value() == "" || year.get_value() == "") {
					frappe.msgprint("Year and Taluka are Mandatory")
					return;
				}
				var flag = false;
				...
				$(
					frappe.render_template("taluka_skeleton")
				).appendTo(page.main);


				await frappe.call({
					method: "asc.dashboards.page.taluka_profile.taluka_profile.get_data",
					args: {
					...
					callback: function (r) {
						console.log("Basic data", r.message);
				...
				// start of enrollment
				frappe.call({
					method: "asc.dashboards.page.taluka_profile.taluka_profile.get_enrollment",
					args: {
						district: district.get_value(),
						year: year.get_value(),
					},
				...
				// start of facility
				frappe.call({
					method: "asc.dashboards.page.taluka_profile.taluka_profile.get_facility",
					args: {
						district: district.get_value(),
						year: year.get_value(),
					},
				...
				// start of staff
				frappe.call({
					method: "asc.dashboards.page.taluka_profile.taluka_profile.get_staff",
					args: {
						district: district.get_value(),
						year: year.get_value(),
					},
				...
				frappe.call({
					method: "asc.dashboards.page.taluka_profile.taluka_profile.year_based_status",
					args: {
						district: district.get_value(),
						year: year.get_value(),
					},
				...
				frappe.call({
					method: "asc.dashboards.page.taluka_profile.taluka_profile.enrollment_ratio",
					args: {
						district: district.get_value(),
						year: year.get_value(),
					},
				...
				frappe.call({
					method: "asc.dashboards.page.taluka_profile.taluka_profile.line_graph_data",
					args: {
						district: district.get_value(),
						year: year.get_value(),
					},
