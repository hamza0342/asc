...
def get_data(year = None):
	...
	SUM(
		CASE WHEN major_reason_closure = "No Enrollment" or school_duration_of_closure = "No Population / No Enrollment" THEN 1 ELSE 0
	END
	) AS no_enrollment,
    SUM(
		CASE WHEN major_reason_closure = "School does not exist" or school_duration_of_closure = "Due to litigation" 
		or school_duration_of_closure = "School ceases to function long time ago and no record available for this school (Not in existence)"
		or major_reason_closure = 'No Population' or major_reason_closure = 'Any Other'
		THEN 1 ELSE 0 
	END
