frappe.pages['transition-rate'].on_page_load = function (wrapper) {
		...
		let fiterbtn = page.add_field({
			...
			click() {
				if (year.get_value() == "" || transition_type.get_value() == "") {
					...
					frappe.call({
						method: "asc.dashboards.page.transition_rate.transition_rate.get_data",
						freeze: true,
						args: {
							year: year.get_value(),
						...
						callback: function (r) {
							...
							$(frappe.render_template("transition_rate")).appendTo(page.main);
