...
def year_based_status(district=None, year=None):
	...
	SUM(no_of_merger_schools) as merged from tabASC where year <= "%s" and district = "%s"
...
def get_enrollment(district=None, year=None):
	...
	WHERE tabASC.docstatus!=2 And tabASC.district = '%s' AND tabASC.year= '%s' 
...
def get_staff(district=None, year=None):
	...
	staff_data_query = """SELECT designation_code AS designation, SUM(CASE when gender = 'Male' then 1 else 0 end) as male_staff,
	 ...
	 FROM
	(Select name from tabASC where tabASC.district = '%s' and tabASC.year= '%s' ) tac Inner JOIN
...
def taluka_data(district=None, year=None):
	...
	taluka_data_query = """Select taluka, 
		...
		from  tabASC where year = '%s' and district = '%s'
		GROUP BY taluka"""%(str(year), district)
...
def taluka_facility(district=None, year=None):
	...
	facility_query_1 = """SELECT taluka,
	SUM(
	IF(
		water_available = 'Yes' ,
		1,
    ...
    from tabASC tac
    where  	docstatus != 2 and status_detail = 'Functional' %s group by taluka order by taluka"""%(condition)

	facility_query_2="""SELECT taluka,
    ...
    FROM (Select name,taluka, year
          from tabASC tac where docstatus != 2 and status_detail= 'Functional' %s) t
    inner join  `tabStatus of Items availability` fac
    on t.name = fac.parent  group by taluka order by taluka """%(condition)
...
def line_graph_data(district=None, year=None):
	...
	as "teachers", SUM(total_enrollment) as enrollment FROM `tabASC` where year <= "%s" and district = "%s"
