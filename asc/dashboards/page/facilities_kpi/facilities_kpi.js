frappe.pages['acilities-kpi'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Facilities KPI',
		single_column: true,
		...
		let fiterbtn = page.add_field({
			...
			click() {
				if (basic_facility.get_value() == "" || year.get_value() == "") {
					...
					frappe.call({
						method: "asc.dashboards.page.facilities_kpi.facilities_kpi.get_data",
						freeze: true,
						args: {
						...
						callback: function (r) {
							...
							$(
								frappe.render_template("facilities_kpi", { 'type': basic_facility.get_value() })
							).appendTo(page.main);
							$("#profile_items").hide()
							$("#barContainer").hide()
		...
		plotOptions: {
			series: {
				events: {
					click: function (e) {
						frappe.call({
							method: "asc.dashboards.page.facilities_kpi.facilities_kpi.get_taluka_data",
							freeze: true,
							args: {
