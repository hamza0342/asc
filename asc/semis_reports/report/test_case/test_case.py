...
def get_data(filters, case_string):
	...
	temp_query = "SELECT  region,district , %s  count(name) FROM tabASC  WHERE docstatus != 2 %s %s" % (str(case_string), conditions, group_by)
	print(temp_query)
	frappe.msgprint(frappe.as_json(temp_query))
	info = frappe.db.sql("SELECT  region,district , %s  count(name) FROM tabASC  WHERE docstatus != 2 %s %s" % (str(case_string), conditions, group_by), filters)
	print(info)
	return info
